...
export class ArrowTool extends WhiteboardTool<ArrowItem> {
  ...
  public override getIcon(): TemplateResult | null {
    return html`${unsafeHTML(getIconSvg("MoveUpRight"))}`;
  }
  ...
  public override handleDrawingMove(x: number, y: number): void {
    ...
    if (!item || !(item instanceof ArrowItem)) {
      return;
    }
  ...
  public override renderToolOptions(item: ArrowItem | null) {
    const whiteboard = this.getSimpleWhiteboardInstance();
